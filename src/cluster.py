...
class ClusterQueue:
	def __init__(self, dirs):
		self.output_path, self.exp_json_dir, self.model_json_dir = dirs[0], dirs[
			1], dirs[2]
		...
		self.cmd_slurm = "sbatch --job-name {} --output {}/%N_%j.log".format(
			jobname, self.output_path)
		...
		for k, v in zip(["output_path", "exp_config", "model_config"],
		                [self.output_path, self.exp_json_dir,
		                 self.model_json_dir]):
...
if __name__ == "__main__":
	...
	if job_submit_mode == "sbatch_array":
		...
		for jj, jobid in enumerate(bash_jobids):
			for config_files in config_dirs[jj * num_jobs_per_bash: min(
					(jj + 1) * num_jobs_per_bash,
					len(config_dirs))]:  # three arguments
				...
				os.system(
					"srun --jobid={} --ntasks=1 --job-name={} --error {}/{}.log python3 classifier.py {} > {}/{}.log &".format(
						jobid, jobname, config_files[0], jobid, cmd_python,
						config_files[0], jobid))
