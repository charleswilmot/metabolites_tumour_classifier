...
def get_hyparams_of_models(model_name="MLP"):
    ...
    if model_name == "MLP":
        ...
        default_parameters = [1e-3, 256, 128, 64, 32]
...
def create_rnn_model(args, inputsize=[288, ]):
    """
    build the graph with the parameters
    :param args:  dict of hyper-parameters
    :param inputsize:
    :return:
    """
    fc_b4rcc1 = args["dim_fnn1"]
    fc_b4rcc2 = args["dim_fnn2"]
    lstm1 = args["dim_rnn1"]
    lstm2 = args["dim_rnn2"]
    model = Sequential()
    # Note that the input-shape must be a tuple containing the image-size.
...
def create_inception_model(args, inputsize=[288,]):
    """
...
def fitness(**params):
    ...
    if MODEL_NAME == "cam":
        model = create_cam_model(params, input_size=INPUT_SIZE)
    elif MODEL_NAME == "rnn":
        model = create_rnn_model(params, inputsize=INPUT_SIZE)
    ...
    if accuracy > best_accuracy or accuracy > 0.80:
        ...
        np.savetxt(os.path.join(results_dir, '{}_rnn_convergence_plot.txt'.format(time_str)), np.array(config_space),
                   fmt="%s", delimiter=",")
        #
